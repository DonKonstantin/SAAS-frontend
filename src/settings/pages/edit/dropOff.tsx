...
export class TransportDropOffEdit implements EditPageConfiguration<"transport_dropoff"> {
    groups: EditFormGroup<"transport_dropoff">[] = [
        {
            title: "Исполнители",
            sizes: {md: 6, xs: 12},
            fields: [
    ...
    editAccessRules: string[] = ["CHANGE_TRANSPORT_DROPOFF", "READ_TRANSPORT_DROPOFF"];
    listPageUrl: PageUrl = {href: "/transport/dropoff"};
    listPageConfig: ListPageConfiguration<"transport_dropoff"> = new TransportDropOffConfiguration();
    editPageUrlGenerator: {(primaryKey: any): PageUrl} = primaryKey => ({as: `/transport/dropoff/edit/${primaryKey}`, href: `/transport/dropoff/edit/[entityId]`});
    entityName: string = "DropOff";
    header: { (primaryKey: any): string } = primaryKey => {
        if (!primaryKey) {
            return `Создать DropOff`
        }

        return `Редактирование DropOff №${primaryKey}`
    };
