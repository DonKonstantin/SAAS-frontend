...
export class TransportContainerRentEdit implements EditPageConfiguration<"transport_container_rent"> {
    groups: EditFormGroup<"transport_container_rent">[] = [
        {
            title: "Исполнители",
            sizes: {md: 6, xs: 12},
            fields: [
    ...
    editAccessRules: string[] = ["CHANGE_TRANSPORT_CONTAINER_RENT", "READ_TRANSPORT_CONTAINER_RENT"];
    listPageUrl: PageUrl = {href: "/transport/container-rent"};
    listPageConfig: ListPageConfiguration<"transport_container_rent"> = new TransportContainerRentConfiguration();
    editPageUrlGenerator: {(primaryKey: any): PageUrl} = primaryKey => ({as: `/transport/container-rent/edit/${primaryKey}`, href: `/transport/container-rent/edit/[entityId]`});
    entityName: string = "Аренда контейнера";
    header: { (primaryKey: any): string } = primaryKey => {
        if (!primaryKey) {
            return `Создать услугу аренды контейнера`
        }

        return `Редактирование услугу аренды контейнера №${primaryKey}`
    };
