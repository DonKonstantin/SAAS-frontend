...
export class CampaignListingConfiguration
  implements ListPageConfiguration<"campaign">
{
  filter: FilterFieldsConfiguration<"campaign"> = {
    ...
    is_active: {
      ...
      filterType: "Switch",
      schema: "campaign",
      title: "pages.campaign.list.filters.is_active",
    },
  };
  listFields: ListFieldsConfiguration<"campaign"> = {
    ...
    additionDataLoader: async (listData: ListFieldRow<"campaign">[]) => {
      const arrayOfCampaign = listData.map(
        (campaign) => campaign.columnValues.id.value
      );
