...
import {
  ListLoadingParameters,
  OrderParameter,
} from "../services/listDataLoader/listLoader/interfaces";
...
type EntityListHocActions<T extends keyof Schemas = keyof Schemas> = {
  ...
  onChangeAdditionData: {
    (callback: {
      <Data extends { [K: string]: any } = { [K: string]: any }>(
        data: Data
      ): Data;
    }): void;
  };
...
const context$ = new BehaviorSubject<EntityListHocContext>(
  new DefaultContext()
);

// Контекст полной перезагрузки данных
const fullDataReloadCtx$ = new Subject<{
...
const onDeleteSubmit: EntityListHocActions["onDeleteSubmit"] = async () => {
  ...
  try {
    const responce = await entityDeleteService().DeleteItemsById(
 ...
 * @param callback
 */
const onChangeAdditionData: EntityListHocActions["onChangeAdditionData"] = (
  callback
) => {
 ...
 * @param value
 */
const onChangeFilterValues: EntityListHocActions["onChangeFilterValues"] = (
  field,
  value
) => {
  const { data } = context$.getValue();
 ...
 */
const setSchema: EntityListHocActions["setSchema"] = (
  schema,
  additionFilter
) => {
...
const initializeSubscriptions = () => {
  ...
  const subscriber = fullDataReloadCtx$
    ...
    .subscribe({
      next: async ({ schema, additionFilter = {} }) => {
        ...
        try {
          const data = await entityListStoreLoader().LoadStoreForConfiguration(
            config,
            additionFilter
          );
          if (!data) {
  ...
  const schemaUpdateSubscription = dataChangeCtx$
    ...
    .subscribe({
      next: async (data) => {
        try {
          const loadedData = await listDataLoader().Load(
            data.currentData.parameters
          );
...
export type WithEntityListHoc<T = {}> = T &
...
type HocType = {
  <T>(
    Component: React.ComponentType<WithEntityListHoc<T>>
