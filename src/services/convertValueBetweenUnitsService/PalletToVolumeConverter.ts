...
export class PalletToVolumeConverter implements _ConvertValueBetweenUnitsProcessorInterface {
    ...
    constructor(unitGroups: UnitGroupData[]) {
        this.baseValueProcessor = new _ConvertationBaseProcessor(
            unitGroups.find(g => g.id === "2") as UnitGroupData,
            (sourceBaseValue, params) => sourceBaseValue * params.length * params.width * params.normativeHeightOfStacking,
            params => params.sourceUnit.unit_group === "4" && params.targetUnit.unit_group === "2",
        )
    }
