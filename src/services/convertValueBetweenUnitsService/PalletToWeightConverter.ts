...
export class PalletToWeightConverter implements _ConvertValueBetweenUnitsProcessorInterface {
    ...
    constructor(unitGroups: UnitGroupData[]) {
        this.baseValueProcessor = new _ConvertationBaseProcessor(
            ...
            (sourceBaseValue, params) => (
                sourceBaseValue * params.length * params.width * this.densityConverter.ConvertToBaseDensity(params.density, params.densityUnitFrom, params.densityUnitTo)
            ),
            params => params.sourceUnit.unit_group === "4" && params.targetUnit.unit_group === "1",
        )
    }
