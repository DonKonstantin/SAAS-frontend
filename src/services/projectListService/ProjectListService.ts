...
import { GetProjectsByIdsQueryRequest, GetProjectsByIdsQueryResnonce } from "./types";
...
class ProjectListService implements ProjectListServiceInterface {
  ...
  async getProjectsByIds(projectIds: string[]): Promise<ProjectData[]> {
    ...
    try {
      const { projects } = await this.#client.Query<GetProjectsByIdsQueryRequest, GetProjectsByIdsQueryResnonce>(
