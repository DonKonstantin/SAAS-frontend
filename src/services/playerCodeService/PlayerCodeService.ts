...
} from "./Querys/CheckCountChannelsByProjectID";
...
export class PlayerCodeService implements PlayerCodeServiceInterface {
  ...
  async getChannels(projectId: string, isActive?: boolean): Promise<ProjectChannel[]> {
    ...
    try {
      ...
      if(limit){
        const response = await this.client.Query<
          GetChannelsByProjectIDQueryParams,
          GetChannelsByProjectIDQueryResponse
        >(new GetChannelsByProjectIDQuery(projectId, limit, isActive), {});
