...
const LoginForm: FC<LoginFormProps> = (props) => {
  const { onToggleForm, onAuthorize, isRequestInProgress } = props;
  ...
  const onSubmitFormFromKeyboard: KeyboardEventHandler<HTMLFormElement> = (
    event
  ) => {
    if (
      event.key !== "Enter" ||
  ...
  const title = `${t(`UI.meta.title.prefix`)}${t(`UI.login.auth.title`)}${t(
    `UI.meta.title.suffix`
  )}`;
  return (
    <>
      <Head>
        ...
        <Grid container spacing={3}>
          <Grid item xs={12}>
            <TextField
              ...
              error={!isAuthValid}
              helperText={
                !isAuthValid ? t(`UI.login.auth.email-invalid`) : undefined
              }
              required
              fullWidth
              InputProps={{
          ...
          <Grid item xs={12}>
            <TextField
              ...
              error={!isAuthValid}
              helperText={
                !isAuthValid ? t(`UI.login.auth.password-invalid`) : undefined
              }
              required
              fullWidth
              InputProps={{
