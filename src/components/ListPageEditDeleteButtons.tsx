...
const ListPageEditDeleteButtons: FC<ListPageEditDeleteButtonsProps> = (
  props
) => {
  const {
    item,
  ...
  const notHasEditAccess =
    permissionCheckEditPermission &&
    !CheckPermission(
      userInfo,
      permissionCheckEditPermission,
      permissionCheckEditLevel
    );
  ...
  return (
    ...
    >
      {!notHasEditAccess && (
        <Tooltip
          title={t(`entity-list.components.actions.edit-tooltip`) as string}
        >
          <IconButton size="small" onClick={onEdit} data-testid="editButton">
      ...
      {!notHasDeleteAccess && (
        ...
        >
          <IconButton size="small" onClick={onDelete} data-testid="deleteButton">
